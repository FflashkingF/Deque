...
class Deque {
  private:
    ...
    static size_t col(size_t ind) {
        return ind % CHUNK;
    }
    static size_t row(size_t ind) {
        return ind / CHUNK;
    }
    size_t afterLast() const {
        return first + sz;
    }
    static size_t devByChunk(size_t cnt) {
        return (cnt + CHUNK - 1) / CHUNK;
    }
    size_t cap() const {
        return arr.size() * CHUNK;
    }
    static T* newChunk() {
        return reinterpret_cast<T*>(new char[sizeof(T) * CHUNK]);
    }
    static void delChunk(T* p) {
        delete[] reinterpret_cast<char*>(p);
    }

    static size_t getStart(size_t capacity) {
        return capacity / 2;
    }

    static size_t startPos(size_t n) {
        return n / 2;
    }
    ...
    size_t getInd(size_t num) const {
        return first + num;
    }
    T* getPtr(size_t num) {
  ...
  public:
    ...
    void push_back(const T& el) {
        if (afterLast() == cap()) {
            std::vector<T*> newarr(
                std::max(static_cast<size_t>(1), arr.size() * 3), nullptr);
